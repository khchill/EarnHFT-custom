...
2. **Train AI Song Song (`train_all_8_models`)**: Chạy đua vũ trang 8 con Bot cùng lúc (4 Low-level và 4 Baseline).
...
2. **Huấn luyện 8 Mô hình (Chạy ngầm song song):**
   ```bash
...
2. **Huấn luyện 8 Mô hình:** 
   Click đúp chuột vào file `train_all_8_models.bat`
...
### 🆘 Xử Lý Sự Cố
- Nếu quá trình chạy 8 mô hình khiến máy của bạn bị treo hoặc báo lỗi Out Of Memory (OOM), hãy mở file `train_all_8_models` lên, dùng dấu `#` (Linux) hoặc `::` (Windows) để comment (vô hiệu hóa) bớt vài con Baseline đi. Chạy từng đợt 4 con sẽ an toàn hơn nếu RAM yếu.
