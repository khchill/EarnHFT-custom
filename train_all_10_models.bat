@echo off
echo === STARTING PARALLEL TRAINING FOR 10 MODELS (WINDOWS) ===
...
start /b "" %PYTHON_CMD% src\EarnHFT_framwork\RL\agent\low_level\ddqn_pes_only_teacher.py --beta 30 --train_data_path data\cleaned_data\BTCUSDT\tardis\train --dataset_name BTCUSDT > log\train\BTCUSDT\low_level\beta_30_only_teacher.log 2>&1
start /b "" %PYTHON_CMD% src\EarnHFT_framwork\RL\agent\low_level\ddqn_pes_only_td.py --beta 30 --train_data_path data\cleaned_data\BTCUSDT\tardis\train --dataset_name BTCUSDT > log\train\BTCUSDT\low_level\beta_30_only_td.log 2>&1

echo [!] System is training 10 models simultaneously.
